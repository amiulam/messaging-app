...
func UpdateUserSessionToken(ctx context.Context, token, refreshToken string, expiredAt time.Time) error {
	return database.DB.Exec("UPDATE user_sessions SET token = ?, token_expired = ? WHERE refresh_token = ?", token, expiredAt, refreshToken).Error
}
